// StrongLog Pro v7.0 - UX Ergonomics, Interactive Guidance & 3D Sci-Fi Engine
...
const app = {
    ...
    buildHologramBodyMesh: (threeInstance = null, heatLevels = null, sceneKey = 'library') => {
        ...
        app.hologramMaterialsPool = materialsPool;
        if (!bodyGroup.userData) bodyGroup.userData = {};
        bodyGroup.userData.materialsPool = materialsPool;
        if (!app.threeScenes[sceneKey]) {
            app.threeScenes[sceneKey] = {};
        }
        app.threeScenes[sceneKey].materialsPool = materialsPool;
        ...
        // 16. Podium Holográfico Circular com THREE.RingGeometry na base
        if (typeof THREE.RingGeometry !== 'undefined') {
            const podiumGeom = new THREE.RingGeometry(0.35, 0.75, 32);
            const podiumMat = new THREE.MeshBasicMaterial({
                color: 0x00FF9D,
                transparent: true,
                opacity: 0.35,
                side: (typeof THREE.DoubleSide !== 'undefined') ? THREE.DoubleSide : 2
            });
            const podiumMesh = new THREE.Mesh(podiumGeom, podiumMat);
            podiumMesh.rotation.x = Math.PI / 2;
            podiumMesh.position.set(0, -1.35, 0);
            podiumMesh.userData = { isPodium: true };
            bodyGroup.add(podiumMesh);
            materialsPool.podiumMat = podiumMat;

            const outerRingGeom = new THREE.RingGeometry(0.78, 0.82, 32);
            ...
            outerRingMesh.position.set(0, -1.35, 0);
            outerRingMesh.userData = { isPodium: true, isOuterRing: true };
            bodyGroup.add(outerRingMesh);
            materialsPool.outerRingMat = outerRingMat;
        }
    ...
    update3DMuscleHighlights: (sceneKey = 'library') => {
        ...
        app.update3DHUD(sceneKey, sceneKey === 'summary' ? 'workout' : 'library');
    },

    update3DHUD: (sceneKey = 'library', mode = 'library') => {
        const hudEl = document.getElementById(`${sceneKey}-3d-hud`) || document.getElementById('library-3d-hud');
        if (!hudEl) return;

        if (mode === 'library' || sceneKey === 'library') {
            if (app.activeMuscleFilter) {
                const grp = app.activeMuscleFilter;
                const grpInfo = app.muscleOntology?.groups?.[grp] || { name: grp };
                const grpName = grpInfo.name || grp;
                const matched = Array.isArray(app.templates)
                    ? app.templates.filter(t => app.matchesMuscleHierarchy(t, grp)).length
                    : 0;

                hudEl.innerHTML = `
                    <div class="hud-telemetry p-2.5 rounded-xl glass border border-[#00FF9D]/30 bg-black/60 backdrop-blur-md text-xs shadow-lg space-y-1">
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-[10px] font-mono uppercase tracking-widest text-[#00FF9D] font-bold">● TELEMETRIA 3D</span>
                            <span class="text-[10px] font-mono bg-[#00FF9D]/20 text-[#00FF9D] px-1.5 py-0.5 rounded font-black">100% ATIVAÇÃO</span>
                        </div>
                        <div class="text-white font-bold text-sm tracking-wide capitalize">${grpName}</div>
                        <div class="text-[10px] text-gray-400 font-medium">${matched} exercícios disponíveis • Estímulo Direto</div>
                    </div>
                `;
            } else {
                hudEl.innerHTML = `
                    <div class="hud-telemetry p-2 rounded-xl glass border border-white/10 bg-black/40 backdrop-blur-md text-[10px] text-gray-400 font-mono flex items-center justify-between">
                        <span>VISUALIZADOR HOLOGRÁFICO</span>
                        <span class="text-[#00FF9D]">19 GRUPOS ATIVOS</span>
                    </div>
                `;
            }
        } else if (mode === 'workout' || sceneKey === 'summary') {
            let totalVol = 0;
            const volByGroup = {};
            if (app.activeWorkout && app.activeWorkout.exercises) {
                app.activeWorkout.exercises.forEach(ex => {
                    const grp = ex.primary_muscle_group || ex.muscleGroup || 'geral';
                    let exVol = 0;
                    (ex.sets || []).forEach(s => {
                    ...
                    totalVol += exVol;
                    volByGroup[grp] = (volByGroup[grp] || 0) + exVol;
                });
            }
            const targetGroup = app.activeMuscleFilter || Object.keys(volByGroup)[0] || 'Geral';
            const groupVol = volByGroup[targetGroup] || 0;
            const pct = totalVol > 0 ? Math.round((groupVol / totalVol) * 100) : 0;
            const grpInfo = app.muscleOntology?.groups?.[targetGroup] || { name: targetGroup };
            const grpName = grpInfo.name || targetGroup;

            hudEl.innerHTML = `
                <div class="hud-telemetry p-2.5 rounded-xl glass border border-cyan-400/30 bg-black/60 backdrop-blur-md text-xs shadow-lg space-y-1">
                    <div class="flex items-center justify-between gap-2">
                        <span class="text-[10px] font-mono uppercase tracking-widest text-cyan-400 font-bold">● VOLUME EFETIVO</span>
                        <span class="text-[10px] font-mono bg-cyan-400/20 text-cyan-400 px-1.5 py-0.5 rounded font-black">${pct}% VOLUME</span>
                    </div>
                    <div class="text-white font-bold text-sm tracking-wide capitalize">${grpName}</div>
                    <div class="text-[10px] text-gray-400 font-medium">${groupVol} kg de ${totalVol} kg na sessão</div>
                </div>
            `;
        }
    },
